...
public class Beach implements Comparable<Beach> {
    ...
    public static void main(String[] args) {
        Beach beach1 = new Beach("Пляж1",4f,5);
        Beach beach2 = new Beach("Пляж2",2f,6);
        System.out.println(beach1.compareTo(beach2));
    }
    public synchronized int compareTo(Beach other){
        int qualitty = Integer.compare(this.quality,other.quality);
        if(qualitty!=0){
            return qualitty;}
        int distanceee = Float.compare(this.distance,other.distance);
        return distanceee;
    }
}
